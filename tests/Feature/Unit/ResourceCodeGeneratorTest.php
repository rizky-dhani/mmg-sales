...
describe('ResourceCodeGenerator', function () {
    it('generates code with correct format for project', function () {
        $generator = app(ResourceCodeGenerator::class);
        $code = $generator->generate('PRJ');

        expect($code)->toMatch('/^MMG-PRJ-\d{6}$/');
    });

    it('generates sequential codes', function () {
        $generator = app(ResourceCodeGenerator::class);

        $code1 = $generator->generate('PRJ');
        $code2 = $generator->generate('PRJ');
        $code3 = $generator->generate('PRJ');

        expect($code1)->toBe('MMG-PRJ-000001');
        expect($code2)->toBe('MMG-PRJ-000002');
        expect($code3)->toBe('MMG-PRJ-000003');
    });

    it('generates different codes for different prefixes', function () {
        $generator = app(ResourceCodeGenerator::class);

        $projectCode = $generator->generate('PRJ');
        $activityCode = $generator->generate('ACT');
        $customerCode = $generator->generate('CST');

        expect($projectCode)->toBe('MMG-PRJ-000001');
    ...
    it('generates unique codes under concurrent-like load', function () {
        ...
        $codes = collect(range(1, 10))->map(fn () => $generator->generate('PRJ'))->toArray();
