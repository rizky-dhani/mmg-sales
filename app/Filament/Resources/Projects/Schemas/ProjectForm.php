                ...
                Grid::make(2)
                    ...
                    ->schema([
                        Section::make('Strategic Management')
                            ->columns(2)
                            ->schema([
                                // Placeholder::make('confidence_level')
                                //     ->label('Confidence Level')
                                //     ->content(function ($record) {
                                //         $level = $record?->confidence_level ?? 0;
                                //         $color = $level >= 80 ? 'bg-success-600' : ($level >= 50 ? 'bg-warning-600' : 'bg-danger-600');

                                //         return new \Illuminate\Support\HtmlString("
                                //             <div class=\"flex items-center gap-x-3\">
                                //                 <div class=\"flex-1 h-3 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800\">
                                //                     <div class=\"h-full {$color} transition-all duration-500\" style=\"width: {$level}%\"></div>
                                //                 </div>
                                //                 <span class=\"text-sm font-medium text-gray-700 dark:text-gray-200\">{$level}%</span>
                                //             </div>
                                //         ");
                                //     }),
                                DatePicker::make('expected_closing_date')
                                    ->label('Expected Closing Date'),
                                TextInput::make('financial_goal')
                                    ->label('Goal')
                                    ->numeric()
                                    ->prefix('IDR'),
                            ]),
