...
class SampleSalesDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $principals = [
            ['code' => 'PRIN01', 'name' => 'Global Pharma Co'],
            ['code' => 'PRIN02', 'name' => 'MedTech Innovations'],
            ['code' => 'PRIN03', 'name' => 'BioLife Sciences'],
        ];

        foreach ($principals as $p) {
            Principal::updateOrCreate(['code' => $p['code']], ['name' => $p['name'], 'is_active' => true]);
        }
        $principalIds = Principal::pluck('id')->toArray();

        // 3. Items
        $itemsData = [
            ['code' => 'ITEM001', 'name' => 'Amoxicillin 500mg', 'price' => 50000, 'principal' => 'PRIN01'],
            ['code' => 'ITEM002', 'name' => 'Paracetamol 500mg', 'price' => 25000, 'principal' => 'PRIN01'],
            ['code' => 'ITEM003', 'name' => 'Surgical Mask 3-Ply', 'price' => 75000, 'principal' => 'PRIN02'],
            ['code' => 'ITEM004', 'name' => 'Digital Thermometer', 'price' => 150000, 'principal' => 'PRIN02'],
            ['code' => 'ITEM005', 'name' => 'Vitamin C 1000mg', 'price' => 120000, 'principal' => 'PRIN03'],
        ];

        foreach ($itemsData as $i) {
            $p = Principal::where('code', $i['principal'])->first();
            Item::updateOrCreate(
                ['internal_code' => $i['code']],
                [
                    'name' => $i['name'],
                    'principal_id' => $p->id,
                    'unit_price' => $i['price'],
